...
class Template
{
  ...
  {
    $filename = Filesystem::clean_filename(str_replace(TEMPLATES_DIR, '', $filename));
    ...
    for ($i = 0; $i < $line_count; $i++) {
      ...
      */
      if ($keyword_type == XS_TAG_INCLUDE) {
        ...
        $line = '<' . '?php ';
        $filehash = Crypt::md5($params_str . $this->include_count . TIMENOW);
  ...
  {
    Filesystem::file_write($code, $filename, false, true, true);
  }
