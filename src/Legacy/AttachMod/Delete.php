...
class Delete
{
  ...
  public static function delete_attachment($post_id_array = 0, $attach_id_array = 0, $page = 0, $user_id = 0)
  {
    ...
    if (is_countable($post_id_array) && is_countable($attach_id_array)) {
        ...
        if ($num_rows == 0) {
          ...
          if ($num_rows != 0) {
            ...
            for ($j = 0; $j < $num_attach; $j++) {
              Attach::unlink_attach($attachments[$j]['physical_filename']);

              if ((int)$attachments[$j]['thumbnail'] == 1) {
                Attach::unlink_attach($attachments[$j]['physical_filename'], MODE_THUMBNAIL);
              }
    ...
    if (is_countable($post_id_array)) {
      ...
      while ($row = DB()->sql_fetchrow($result)) {
        Attach::attachment_sync_topic($row['topic_id']);
      }
      DB()->sql_freeresult($result);
    }
  }
