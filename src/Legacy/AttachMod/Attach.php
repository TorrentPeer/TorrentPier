...
class Attach
{
   ...
   * @return string|void
   */
  private static function base64_pack($number)
  {
   ...
   * @return float|int
   */
  private static function base64_unpack($string)
  {
    $chars = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ+-';
    $base = strlen($chars);
   ...
   * @return string
   */
  public static function auth_pack($auth_array)
  {
    $one_char_encoding = '#';
    $two_char_encoding = '.';
    ...
    foreach ($auth_array as $i => $iValue) {
      $val = self::base64_pack((int)$auth_array[$i]);
   ...
   * @return array
   */
  public static function auth_unpack($auth_cache)
  {
    $one_char_encoding = '#';
    $two_char_encoding = '.';
    ...
    for ($pos = 0, $posMax = strlen($auth_cache); $pos < $posMax; $pos += $auth_len) {
      ...
      $forum_id = self::base64_unpack($forum_auth);
      $auth[] = (int)$forum_id;
    }
    return $auth;
  }
   ...
   * @return bool
   */
  public static function is_forum_authed($auth_cache, $check_forum_id)
  {
    $one_char_encoding = '#';
    $two_char_encoding = '.';
    ...
    for ($pos = 0, $posMax = strlen($auth_cache); $pos < $posMax; $pos += $auth_len) {
      ...
      $forum_auth = substr($auth_cache, $pos, $auth_len);
      $forum_id = (int)self::base64_unpack($forum_auth);
   ...
   * @return bool
   */
  public static function unlink_attach($filename, $mode = false)
  {
  ...
  public static function attachment_exists($filename)
  {
    ...
    if (!@file_exists(@self::amod_realpath($upload_dir . '/' . $filename))) {
      return false;
    }
   ...
   * @return bool
   */
  public static function thumbnail_exists($filename)
  {
    global $upload_dir, $attach_config;

    $filename = basename($filename);

    if (!@file_exists(@self::amod_realpath($upload_dir . '/' . THUMB_DIR . '/t_' . $filename))) {
      return false;
    }
  ...
  public static function physical_filename_already_stored($filename)
  {
    ...
    $sql = 'SELECT attach_id
		...
		WHERE physical_filename = '" . self::attach_mod_sql_escape($filename) . "'
		LIMIT 1";
   ...
   * @return mixed
   */
  public static function get_extension_informations()
  {
   ...
   * @return false|string
   */
  public static function delete_extension($filename)
  {
   ...
   */
  public static function amod_realpath($path)
  {
   ...
   * @private
   */
  public static function _set_var(&$result, $var, $type, $multibyte = false)
  {
    settype($var, $type);
    $result = $var;
   ...
   * @return array
   */
  public static function get_var($var_name, $default, $multibyte = false)
  {
    ...
    if (is_array($var)) {
      ...
      foreach ($_var as $k => $v) {
        if (is_array($v)) {
          foreach ($v as $_k => $_v) {
            self::_set_var($k, $k, $key_type);
            self::_set_var($_k, $_k, $key_type);
            self::_set_var($var[$k][$_k], $_v, $type, $multibyte);
          }
        } else {
          self::_set_var($k, $k, $key_type);
          self::_set_var($var[$k], $v, $type, $multibyte);
        }
      }
    } else {
      self::_set_var($var, $var, $type, $multibyte);
    }
   ...
   */
  public static function attach_mod_sql_escape($text)
  {
   ...
   *
   * @param $query
   * @param false $assoc_ary
   * @return false|mixed|string
   */
  public static function attach_mod_sql_build_array($query, $assoc_ary = false)
  {
    ...
    if ($query == 'INSERT' || $query == 'INSERT_SELECT') {
      foreach ($assoc_ary as $key => $var) {
        ...
        if (null === $var) {
          ...
          $values[] = "'" . self::attach_mod_sql_escape($var) . "'";
      ...
      foreach ($assoc_ary as $key => $var) {
        if (null === $var) {
          ...
          $values[] = "$key = '" . self::attach_mod_sql_escape($var) . "'";
        } else {
