...
class Includes
{
   ...
   * @param $s_auth_can
   */
  public static function attach_build_auth_levels($is_auth, &$s_auth_can)
  {
  ...
  public static function attachment_quota_settings($admin_mode, $mode, $submit = false)
  {
    ...
    if ($admin_mode == 'user') {
      ...
      if (!$submit && $mode != 'save') {
        $user_id = Attach::get_var(POST_USERS_URL, 0);
        $u_name = Attach::get_var('username', '');
    ...
    if ($admin_mode == 'user' && !$submit && $mode != 'save') {
      ...
      $template->assign_vars([
        'S_SELECT_UPLOAD_QUOTA' => Selects::quota_limit_select('user_upload_quota', $upload_quota),
        'S_SELECT_PM_QUOTA' => Selects::quota_limit_select('user_pm_quota', $pm_quota),
      ]);
    }

    if ($admin_mode == 'user' && $submit && @$_POST['delete_user']) {
      Admin::process_quota_settings($admin_mode, $user_id, QUOTA_UPLOAD_LIMIT, 0);
      Admin::process_quota_settings($admin_mode, $user_id, QUOTA_PM_LIMIT, 0);
    } elseif ($admin_mode == 'user' && $submit && $mode == 'save') {
      // Get the contents
      $upload_quota = Attach::get_var('user_upload_quota', 0);
      $pm_quota = Attach::get_var('user_pm_quota', 0);

      Admin::process_quota_settings($admin_mode, $user_id, QUOTA_UPLOAD_LIMIT, $upload_quota);
      Admin::process_quota_settings($admin_mode, $user_id, QUOTA_PM_LIMIT, $pm_quota);
    }
    ...
    if ($admin_mode == 'group' && !$submit && isset($_POST['edit'])) {
      // Get group id again
      $group_id = Attach::get_var(POST_GROUPS_URL, 0);
      ...
      $template->assign_vars([
        'S_SELECT_UPLOAD_QUOTA' => Selects::quota_limit_select('group_upload_quota', $upload_quota),
        'S_SELECT_PM_QUOTA' => Selects::quota_limit_select('group_pm_quota', $pm_quota),
      ]);
    }
    ...
    if ($admin_mode == 'group' && $submit && isset($_POST['group_delete'])) {
      ...
      Admin::process_quota_settings($admin_mode, $group_id, QUOTA_UPLOAD_LIMIT, 0);
      Admin::process_quota_settings($admin_mode, $group_id, QUOTA_PM_LIMIT, 0);
    } elseif ($admin_mode == 'group' && $submit) {
      $group_id = Attach::get_var(POST_GROUPS_URL, 0);

      // Get the contents
      $upload_quota = Attach::get_var('group_upload_quota', 0);
      $pm_quota = Attach::get_var('group_pm_quota', 0);

      Admin::process_quota_settings($admin_mode, $group_id, QUOTA_UPLOAD_LIMIT, $upload_quota);
      Admin::process_quota_settings($admin_mode, $group_id, QUOTA_PM_LIMIT, $pm_quota);
    }
  }
}
