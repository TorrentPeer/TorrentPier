...
class Attach
{
  ...
  {
    $this->add_attachment_body = AttachMod::get_var('add_attachment_body', 0);
    $this->posted_attachments_body = AttachMod::get_var('posted_attachments_body', 0);

    $this->file_comment = AttachMod::get_var('filecomment', '');
    $this->attachment_id_list = AttachMod::get_var('attach_id_list', [0]);
    $this->attachment_comment_list = AttachMod::get_var('comment_list', ['']);
    $this->attachment_filesize_list = AttachMod::get_var('filesize_list', [0]);
    $this->attachment_filetime_list = AttachMod::get_var('filetime_list', [0]);
    $this->attachment_filename_list = AttachMod::get_var('filename_list', ['']);
    $this->attachment_extension_list = AttachMod::get_var('extension_list', ['']);
    $this->attachment_mimetype_list = AttachMod::get_var('mimetype_list', ['']);
    ...
    $this->attachment_list = AttachMod::get_var('attachment_list', ['']);
    $this->attachment_thumbnail_list = AttachMod::get_var('attach_thumbnail_list', [0]);
  }
  ...
  public function handle_attachments($mode)
  {
    ...
    // Get Attachments
    $attachments = AttachMod::get_attachments_from_post($post_id);
    ...
    if ($preview || $refresh || $error) {
      ...
      if ($delete_attachment || $delete_thumbnail) {
        // store old values
        $actual_id_list = AttachMod::get_var('attach_id_list', [0]);
        $actual_comment_list = AttachMod::get_var('comment_list', ['']);
        $actual_filename_list = AttachMod::get_var('filename_list', ['']);
        $actual_extension_list = AttachMod::get_var('extension_list', ['']);
        $actual_mimetype_list = AttachMod::get_var('mimetype_list', ['']);
        $actual_filesize_list = AttachMod::get_var('filesize_list', [0]);
        $actual_filetime_list = AttachMod::get_var('filetime_list', [0]);

        $actual_list = AttachMod::get_var('attachment_list', ['']);
        $actual_thumbnail_list = AttachMod::get_var('attach_thumbnail_list', [0]);
          ...
          for ($i = 0, $iMax = \count($actual_list); $i < $iMax; $i++) {
            ...
            if ($restore) {
              ...
              if ($actual_id_list[$i] == '0') {
                AttachMod::unlink_attach($actual_list[$i]);

                if ($actual_thumbnail_list[$i] == 1) {
                  AttachMod::unlink_attach($actual_list[$i], MODE_THUMBNAIL);
                }
              } else {
                Delete::delete_attachment($post_id, $actual_id_list[$i], $this->page);
              }
            } elseif ($del_thumb) {
        ...
        if ($update_attachment) {
          ...
          if (!$error) {
            ...
            $sql = 'UPDATE ' . BB_ATTACHMENTS_DESC . ' SET ' . AttachMod::attach_mod_sql_build_array('UPDATE', $sql_ary) . '
							WHERE attach_id = ' . (int)$attachment_id;
            ...
            AttachMod::unlink_attach($row['physical_filename']);

            if ((int)$row['thumbnail'] === 1) {
              AttachMod::unlink_attach($row['physical_filename'], MODE_THUMBNAIL);
            }
      ...
      for ($i = 0, $iMax = \count($this->attachment_list); $i < $iMax; $i++) {
        if ($this->attachment_id_list[$i]) {
          ...
          $sql = 'UPDATE ' . BB_ATTACHMENTS_DESC . "
						SET comment = '" . @AttachMod::attach_mod_sql_escape($this->attachment_comment_list[$i]) . "'
          ...
          $sql = 'INSERT INTO ' . BB_ATTACHMENTS_DESC . ' ' . AttachMod::attach_mod_sql_build_array('INSERT', $sql_ary);
          ...
          $sql = 'INSERT INTO ' . BB_ATTACHMENTS . ' ' . AttachMod::attach_mod_sql_build_array('INSERT', $sql_ary);
  ...
  public function upload_attachment()
  {
    ...
    if ($this->post_attach) {
      ...
      $this->extension = strtolower(AttachMod::get_extension($this->filename));
      ...
      $sql = 'SELECT g.allow_group, g.max_filesize, g.cat_id, g.forum_permissions
				...
				WHERE g.group_id = e.group_id
					AND e.extension = '" . AttachMod::attach_mod_sql_escape($this->extension) . "'
				LIMIT 1";
      ...
      if (!$error && !IS_ADMIN && !AttachMod::is_forum_authed($auth_cache, $forum_id) && trim($auth_cache)) {
        $error = true;
        if (!empty($error_msg)) {
          $error_msg .= '<br />';
        }
      ...
      if (!$error && !IS_ADMIN && $cat_id === IMAGE_CAT) {
        [$width, $height] = Filetypes::image_getdimension($upload_dir . '/' . $this->attach_filename);
      ...
      if ($error) {
        AttachMod::unlink_attach($this->attach_filename);
        AttachMod::unlink_attach($this->attach_filename, MODE_THUMBNAIL);
        $this->post_attach = false;
      }
    }
  }
  ...
  public function move_uploaded_attachment($upload_mode, $file)
  {
    ...
    if (!$error && $this->thumbnail === 1) {
      ...
      $dest_file = AttachMod::amod_realpath($upload_dir);
      $dest_file .= '/' . THUMB_DIR . '/t_' . basename($this->attach_filename);

      if (!Thumbs::create_thumbnail($source, $dest_file, $this->type)) {
        if (!$file || !Thumbs::create_thumbnail($file, $dest_file, $this->type)) {
          $this->thumbnail = 0;
        }
      }
    }
