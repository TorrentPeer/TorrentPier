...
$userdata = $images = $lang = $nav_links = $bf = $attach_config = [];
...
$user_ip = \TorrentPier\Helpers\IPHelper::encode_ip($client_ip);
...
if ((empty($_POST) && !defined('IN_ADMIN') && !defined('IN_AJAX') && !file_exists(CRON_RUNNING) && ($bb_cfg['cron_enabled'] || defined('START_CRON'))) || defined('FORCE_CRON')) {
  if (TIMENOW - $bb_cfg['cron_last_check'] > $bb_cfg['cron_check_interval']) {
    ...
    \TorrentPier\Legacy\Logging::bb_log(date('H:i:s - ') . getmypid() . ' -x-- DB-LOCK try' . LOG_LF, CRON_LOG_DIR . '/cron_check');

    if (DB()->get_lock('cron', 1)) {
      \TorrentPier\Legacy\Logging::bb_log(date('H:i:s - ') . getmypid() . ' --x- DB-LOCK OBTAINED !!!!!!!!!!!!!!!!!' . LOG_LF, CRON_LOG_DIR . '/cron_check');
      ...
      if (defined('IN_CRON')) {
        \TorrentPier\Legacy\Logging::bb_log(date('H:i:s - ') . getmypid() . ' --x- ALL jobs FINISHED *************************************************' . LOG_LF, CRON_LOG_DIR . '/cron_check');
      }
...
if (($bb_cfg['board_disable'] || file_exists(BB_DISABLED)) && !defined('IN_ADMIN') && !defined('IN_AJAX') && !defined('IN_LOGIN')) {
  \TorrentPier\Legacy\Common\Http::setCode(503);
