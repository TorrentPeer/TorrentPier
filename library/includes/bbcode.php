...
// Acronym
  $bbcode_tpl['acronym_open'] = <<<HTML
	<abbr class="post-acronym">
HTML;

  $bbcode_tpl['acronym_title_open'] = <<<HTML
	<abbr title="\\1" class="post-acronym">
HTML;

  $bbcode_tpl['acronym_close'] = <<<HTML
    </abbr>
HTML;

// Thumbnail
  $bbcode_tpl['thumbnail'] = <<<HTML
    <a href="\\1" onclick="return hs.expand(this)" rel="topic" class="highslide"><img src="\\1" border="0" style="max-width:200px; max-height:120px;"></a>
HTML;
...
function strip_quotes($text)
{
  ...
  if (is_countable($start_pos) && (count($start_pos) == 0)) {
    return $text;
  }
  ...
  if (is_countable($end_pos) && (count($end_pos) == 0)) {
    return $text;
  }
...
function extract_search_words($text)
{
  ...
  if (is_countable($text) && (count($text) > $max_words_count)) {
