...
if ($releasing) {
  foreach ($releasing as $i => $row) {
    ...
    $template->assign_block_vars('released', [
      ...
      'SPEED_UP' => ($row['speed_up']) ? \TorrentPier\Legacy\Filesystem::humn_size($row['speed_up'], 0, 'KB') . '/s' : '-',
    ]);
...
if ($leeching) {
  foreach ($leeching as $i => $row) {
    ...
    $template->assign_block_vars('leech', [
      ...
      'TOPIC_SEEDERS' => ($row['seeders']) ?: 0,
      'TOPIC_LEECHERS' => ($row['leechers']) ?: 0,
      'SPEED_DOWN' => ($row['speed_down']) ? \TorrentPier\Legacy\Filesystem::humn_size($row['speed_down'], 0, 'KB') . '/s' : '-',
    ]);
