...
foreach ($cron_jobs as $job) {
  ...
  if (file_exists($job_script)) {
    ...
    if ($cron_write_log) {
      ...
      $msg[] = date('m-d');
      $msg[] = date('H:i:s');
      $msg[] = sprintf('%-4s', round(\TorrentPier\Helpers\BaseHelper::sys('la'), 1));
      $msg[] = sprintf('%05d', getmypid());
      $msg[] = $job['cron_title'];
      $msg = implode(LOG_SEPR, $msg);
      \TorrentPier\Legacy\Logging::bb_log($msg . LOG_LF, CRON_LOG_DIR . '/' . CRON_LOG_FILE);
    }
    ...
    if ($job['log_sql_queries']) {
      DB()->log_next_query(0);
      \TorrentPier\Legacy\Logging::bb_log(LOG_LF, $cron_sql_log_file);
    }
    ...
    if ($cron_write_log) {
      ...
      if ($cron_runtime_log) {
        ...
        \TorrentPier\Legacy\Logging::bb_log($cron_runtime_log . LOG_LF, CRON_LOG_DIR . '/' . basename($runtime_log_file));
      }
    }
    ...
    \TorrentPier\Legacy\Logging::bb_log($cron_err_msg, 'cron_error');
  }
}
