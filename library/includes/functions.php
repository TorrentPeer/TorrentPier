...
# 'user_opt_name'      =>     НАСТРОЙКИ используемые пользователями
  ...
  'user_show_zodiac' => 16, // Знак зодиака
 ...
 * @throws \Exception
 */
function groupname($select_name, $selected_group, $groupid)
{
  global $lang;
...
/**
 * Choose russian word declension based on numeric [from dklab.ru]
 * Example for $expressions: ["ответ", "ответа", "ответов"]
 *
 * @param $int
 * @param $expressions
 * @param string $format
...
function declension($int, $expressions, $format = '%1$s %2$s')
{
  ...
  if (is_countable($expressions) && (count($expressions) < 3)) {
...
function bt_show_ip($ip, $port = '')
{
  ...
  if (IS_AM) {
    $ip = \TorrentPier\Helpers\IPHelper::decode_ip($ip);
    $ip .= ($port) ? ":$port" : '';
    return $ip;
  }

  return ($bb_cfg['bt_show_ip_only_moder']) ? false : \TorrentPier\Helpers\IPHelper::ip_anonymize($ip);
}

/**
 * Подготовка порта перед отображением
 *
 * @param $port
 * @return false|mixed
 */
...
/**
 * Получает username на основе ID
 *
 * @param $user_id
 * @return array|false|mixed
 * @throws \Exception
...
/**
 * Получает ID пользователя на основе username
 *
 * @param $username
 * @return false|mixed
 * @throws \Exception
 ...
 * @return string
 */
function bb_ltrim($str, $charlist = false): string
{
  if ($charlist === false) {
    return ltrim($str);
  }
 ...
 * @return string
 */
function bb_rtrim($str, $charlist = false): string
{
  if ($charlist === false) {
    return rtrim($str);
  }
 ...
 *
 * @param $u
 * @param false $force_name
 * @param false $allow_guest
 * @return array|bool|mixed|string|null
 * @throws \Exception
 */
...
/**
 * Запуск и инициализация шаблонизатора
 *
 * @return string[]
 */
function setup_style()
{
  ...
  $tpl_dir_name = basename($bb_cfg['templating']['tpl_name']);
  $base_dir = 'styles/' . basename(TEMPLATES_DIR) . '/' . $tpl_dir_name;
  ...
  $template->assign_vars([
    ...
    'STYLESHEET' => make_url($base_dir . '/css/' . $bb_cfg['templating']['stylesheet']),
    'SCRIPTS' => make_url($base_dir . '/' . $bb_cfg['templating']['javascript']),
    'EXT_LINK_NEW_WIN' => $bb_cfg['ext_link_new_win'],
    'TPL_DIR' => make_url($base_dir . '/'),
    'SITE_URL' => make_url('/'),
  ]);
...
/**
 * Вывод возраста на основе даты рождения
 *
 * @param $date
 * @return mixed
 */
...
function bb_die($msg_text)
{
  ...
  if (!defined('PAGE_HEADER_SENT')) {
    if (empty($template)) {
      $template = new TorrentPier\Legacy\Template(BB_ROOT . "templates/{$bb_cfg['templating']['tpl_name']}");
    }
    if (empty($theme)) {
      $theme = setup_style();
    }
...
/**
 * Обновление страницы по средствам meta
 *
 * @param $url
 * @param int $time
 */
 ...
 * @return string
 */
function topic_attachment_image($switch_attachment): string
{
  global $is_auth;
...
function get_title_match_topics($title_match_sql, array $forum_ids = [])
{
  ...
  if ($bb_cfg['search_engine_type'] == 'sphinx') {
    if (!isset($sphinx)) {
      $sphinx = new \TorrentPier\Legacy\Sphinx();
    }

    $where = $title_match ? 'topics' : 'posts';

    $sphinx->setServer($bb_cfg['sphinx']['host'], $bb_cfg['sphinx']['port']);
    if ($forum_ids) {
    ...
    if ($result = $sphinx->query($title_match_sql, $where, $userdata['username'] . ' (' . CLIENT_IP . ')')) {
      ...
      \TorrentPier\Legacy\Sphinx::log_sphinx_error('Error', $error, $title_match_sql);
    }
    if ($warning = $sphinx->getLastWarning()) {
      \TorrentPier\Legacy\Sphinx::log_sphinx_error('Warning', $warning, $title_match_sql);
    }
...
/**
 * Генерирует ссылку на профиль на основе массива userdata
 *
 * @param $data
 * @return string
 * @throws \Exception
...
/**
 * Возвращает иконку золото/серебро
 *
 * @param $type
 * @return string
 */
...
/**
 * Возвращает знак зодиака пользователя на основе его даты рождения
 *
 * @param $birthday
 * @param string $mode
 * @return mixed
...
function get_zodiac($birthday, $mode = 'full')
{
  ...
  if (!empty($birthday)) {
    ...
    $bmonth = $birthday[1]; // месяц
    $bday = $birthday[2]; // день
