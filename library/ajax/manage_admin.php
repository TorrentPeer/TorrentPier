...
switch ($mode) {
  ...
  case 'indexer':
    ...
    exec("indexer --config {$bb_cfg['sphinx']['path']} --all --rotate", $result);

    if (!is_file($bb_cfg['sphinx']['path'] . ".log")) {
      file_put_contents($bb_cfg['sphinx']['path'] . ".log", "##############################" . date("H:i:s", TIMENOW) . "##############################\r\n\r\n\r\n\r\n", FILE_APPEND);
    }

    file_put_contents($bb_cfg['sphinx']['path'] . ".log", "##############################" . date("H:i:s", TIMENOW) . "##############################\r\n", FILE_APPEND);

    foreach ($result as $row) {
      file_put_contents($bb_cfg['sphinx']['path'] . ".log", $row . "\r\n", FILE_APPEND);
    }

    file_put_contents($bb_cfg['sphinx']['path'] . ".log", "\r\n", FILE_APPEND);
    file_put_contents($bb_cfg['sphinx']['path'] . ".log", "\r\n", FILE_APPEND);
  ...
  case 'admin_note':
    ...
    bb_update_config(['admin_note' => $note]);

    $this->response['note_saved'] = $lang['SAVED'];
