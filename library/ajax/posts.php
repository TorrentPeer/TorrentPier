...
switch ($this->request['type']) {
    ...
    if ((mb_strlen($post['post_text'], 'UTF-8') > 1000) || $post['post_attachment'] || ($post['topic_first_post_id'] == $post_id)) {
      ...
      $this->response['text'] = '
				<form action="' . POSTING_URL . '" method="post" name="post">
					' . $hidden_form . '
					<div class="buttons mrg_4 floatL">
                        ...
                        <input type="button" value="mp4" name="codeMP4" title="' . $lang['MP4_TITLE'] . '" style="width: 40px;" />
						...
						<input type="button" value="' . $lang['ACRONYM'] . '" name="codeAcronym" title="' . $lang['ACRONYM'] . '" style="width: 65px;" />
						...
						<input type="button" value="nfo" name="codeInfo" title="' . $lang['INFO_TITLE'] . '" style="width: 40px;" />
					</div>
  ...
  default:
    $this->ajax_die("Invalid type: " . $this->request['type']);
}
