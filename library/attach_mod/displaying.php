...
function init_complete_extensions_data()
{
  ...
  if (!$extension_informations = \TorrentPier\Legacy\AttachMod\Attach::get_extension_informations()) {
    $GLOBALS['datastore']->update('attach_extensions');
    $extension_informations = \TorrentPier\Legacy\AttachMod\Attach::get_extension_informations();
  }
  $allowed_extensions = [];
...
function init_display_post_attachments($switch_attachment)
{
  ...
  if (is_countable($post_id_array) && (count($post_id_array) == 0)) {
    return;
  }

  $rows = \TorrentPier\Legacy\AttachMod\Attach::get_attachments_from_post($post_id_array);
  $num_rows = count($rows);
 ...
 * used by all displaying functions, the Data was collected before, it's only dependent on the template used. :)
...
function display_attachments($post_id)
{
  ...
  for ($i = 0; $i < $num_attachments; $i++) {
    ...
    if (!in_array($attachments['_' . $post_id][$i]['extension'], $allowed_extensions)) {
      ...
      $template->assign_block_vars('postrow.attach.denyrow', [
    ...
    if (!$denied || IS_ADMIN) {
      ...
      if (@(int)$display_categories[$attachments['_' . $post_id][$i]['extension']] == IMAGE_CAT && (int)$attach_config['img_display_inlined']) {
        if ((int)$attach_config['img_link_width'] != 0 || (int)$attach_config['img_link_height'] != 0) {
          [$width, $height] = \TorrentPier\Legacy\AttachMod\Filetypes::image_getdimension($filename);
