...
if ($is_moderator) {
  ...
  if ($submit) {
    if (!empty($_FILES['avatar']['name']) && $bb_cfg['group_avatars']['up_allowed']) {
      ...
      if (DB()->query("UPDATE " . BB_GROUPS . " SET avatar_ext_id = $avatar_ext_id WHERE group_id = $group_id")) {
        reload();
      }
    }
  }
  ...
  $template->assign_vars([
    ...
    'AVATAR_IMG' => \TorrentPier\Legacy\Avatar::getAvatar(true, GROUP_AVATAR_MASK . $group_id, $group_info['avatar_ext_id']),
  ]);
