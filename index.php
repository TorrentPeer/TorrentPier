...
$cache_name = 'index_sql_' . \TorrentPier\Legacy\Crypt::md5($sql);
...
foreach ($cat_forums as $cid => $c) {
  ...
  foreach ($c['f'] as $fid => $f) {
    ...
    $template->assign_block_vars('c.f', [
      ...
      'FORUM_DESC' => is_countable($orig_word) ? preg_replace($orig_word, $replacement_word, $f['forum_desc']) : $f['forum_desc'],
    ...
    if ($f['last_post_id']) {
      $template->assign_block_vars('c.f.last', [
        ...
        'LAST_TOPIC_TIP' => is_countable($orig_word) ? preg_replace($orig_word, $replacement_word, $f['last_topic_title']) : $f['last_topic_title'],
        'LAST_TOPIC_TITLE' => wbr(str_short(is_countable($orig_word) ? preg_replace($orig_word, $replacement_word, $f['last_topic_title']) : $f['last_topic_title'], $last_topic_max_len)),
...
if ($bb_cfg['show_latest_news']) {
  ...
  foreach ($latest_news as $news) {
    $template->assign_block_vars('news', [
      ...
      'NEWS_TITLE' => str_short(is_countable($orig_word) ? preg_replace($orig_word, $replacement_word, $news['topic_title']) : $news['topic_title'], $bb_cfg['max_news_title']),
...
if ($bb_cfg['show_network_news']) {
  ...
  foreach ($network_news as $net) {
    $template->assign_block_vars('net', [
      ...
      'NEWS_TITLE' => str_short(is_countable($orig_word) ? preg_replace($orig_word, $replacement_word, $net['topic_title']) : $net['topic_title'], $bb_cfg['max_net_title']),
