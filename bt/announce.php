...
if (!\TorrentPier\Helpers\IPHelper::verify_ip($ip)) {
  msg_die("Invalid IP: $ip");
}
// Convert IP to HEX format
$ip_sql = \TorrentPier\Helpers\IPHelper::encode_ip($ip);

// Peer unique id
$peer_hash = \TorrentPier\Legacy\Crypt::md5(
...
if (!$output) {
  ...
  if ($compact_mode) {
    ...
    foreach ($rowset as $peer) {
      $peers .= pack('Nn', \TorrentPier\Helpers\IPHelper::encode_ip(\TorrentPier\Helpers\IPHelper::decode_ip($peer['ip'])), $peer['port']);
    }
  } else {
    $peers = [];
    ...
    foreach ($rowset as $peer) {
      $peers[] = [
        'ip' => \TorrentPier\Helpers\IPHelper::decode_ip($peer['ip']),
        'port' => (int)$peer['port'],
      ];
    }
  }
