...
// Who is looking topic
if ($userdata['user_id'] != GUEST_UID && $bb_cfg['show_who_is_looking']) {
  DB()->query("UPDATE " . BB_USERS . " SET curpage_topic = $topic_id, curpage_topic_time = " . TIMENOW . " WHERE user_id = {$userdata['user_id']}");
  ...
  $template->assign_vars([
    'LOOKING_LIST' => is_countable($looking_list) ? ($lang['WHOIS_LOOKING'] . '&nbsp; ' . implode(",\n", $looking_list)) : '',
  ]);
}
...
if ($t_data['topic_show_first_post'] && $start) {
  $first_post = DB()->fetch_rowset("
		SELECT
			...
			u.avatar_ext_id,
			u.user_opt, u.user_gender, u.user_birthday, u.user_prov, u.user_reputation,
...
$template->assign_vars([
  ...
  'REPUTATION_ON' => $bb_cfg['reputation']['disabled'] ? false : true,
...
for ($i = 0; $i < $total_posts; $i++) {
  ...
  $poster_prov = ($postrow[$i]['user_prov'] && $poster_id != GUEST_UID) ? $postrow[$i]['user_prov'] : '';
  ...
  $rg_avatar = \TorrentPier\Legacy\Avatar::getAvatar(true, GROUP_AVATAR_MASK . $rg_id, $postrow[$i]['rg_avatar_id']);
  ...
  if (!$user->opt_js['h_av'] && $poster_id != GUEST_UID) {
    $poster_avatar = \TorrentPier\Legacy\Avatar::getAvatar(false, $poster_id, $postrow[$i]['avatar_ext_id'], !bf($postrow[$i]['user_opt'], 'user_opt', 'dis_avatar'));
  }
  ...
  $template->assign_block_vars('postrow', [
    ...
    'REPUTATION' => $postrow[$i]['user_reputation'],
    'REPUTATION_COLOR' => ($postrow[$i]['user_reputation'] < 0) ? 'red' : 'green',
