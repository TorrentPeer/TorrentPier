...
function return_msg_mcp($status_msg)
{
  ...
  if (is_countable($req_topics) && (count($req_topics) == 1)) {
  ...
  if ($topic_id && $mode != 'delete') {
    ...
    $message .= '<br /><br />';
  } elseif (is_countable($req_topics) && (count($req_topics) != 1)) {
...
switch ($mode) {
  ...
  case 'ip':
    ...
    if (!$ip_this_post = \TorrentPier\Helpers\IPHelper::decode_ip($post_row['poster_ip'])) {
    ...
    if ($row = DB()->sql_fetchrow($result)) {
      ...
      do {
        ...
        if (!$ip = \TorrentPier\Helpers\IPHelper::decode_ip($row['poster_ip'])) {
