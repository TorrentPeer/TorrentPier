...
function manage_group(mode, value) {
    ...
    $('div#avatar').html(data.group_avatar);
	}
}
</script>
	...
	<tr>
		<td>
			...
			<div id="avatar" align="center">
				<p>{AVATAR_IMG}</p><br />
				<p><input type="button" name="delete_avatar" value="{L_AVATAR_DELETE}" onclick="if (window.confirm('{L_AVATAR_DELETE}?')){ manage_group(this.name,this.value); }" /></p>
			</div>
		</td>
		<td valign="top">
			{AVATAR_EXPLAIN}
        ...
        <div class="spacer_4"></div>
          ...
          <input type="file" name="avatar" />
					<input class="mainoption" type="submit" name="submit" value="{L_SAVE}" />
				</form>
			<!-- ENDIF -->
		</td>
	</tr>
