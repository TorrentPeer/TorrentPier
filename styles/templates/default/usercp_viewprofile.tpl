...
ajax.callback.change_user_rank = function (data) {
  ...
  $('#rank-image').html(data.rank_image);
};
...
<tr>
    ...
    <div id="rank-image" class="mrg_6">{RANK_IMAGE}</div>
	...
	<td class="row1" valign="top" width="70%">
      ...
      <tr>
        ...
        <td>{USER_STATUS}</td>
            ...
            <!-- IF PROFILE_USER || IS_ADMIN -->[ <a class="med" href="{U_WATCHED_TOPICS_PROFILE}">{L_WATCHED_TOPICS}</a> ]<!-- ENDIF -->
      ...
      <tr>
        <th>{L_REPUTATION}:</th>
        <td id="user_reputation"><b style="color:{REPUTATION_COLOR}" class="editable">{REPUTATION}</b>&nbsp;<a href="#" onclick="ajax.reputation({PROFILE_USER_ID}, 'add'); return false;" class="small">[+]</a>&nbsp;<a href="#" onclick="ajax.reputation({PROFILE_USER_ID}, 'del'); return false;" class="small">[-]</a></td>
      </tr>
      <!-- ENDIF -->
      ...
      <!-- ENDIF -->
			...
			<tr>
				<th>{L_GENDER}:</th>
				<td id="user_gender"><span class="editable bold">{GENDER}</span></td>
			</tr>
			<!-- ENDIF -->
			<!-- IF BIRTHDAY -->
			<tr>
				<th>{L_BIRTHDAY}:</th>
				<td id="user_birthday"><span class="editable bold">{BIRTHDAY}</span> <!-- IF SHOW_ZODIAC -->{SHOW_ZODIAC}<!-- ENDIF --></td>
			</tr>
			<tr>
				<th>{L_AGE}:</th>
				<td><b>{AGE}</b></td>
