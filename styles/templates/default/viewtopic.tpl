...
<tr>
	...
	<!-- ELSE -->
		...
		<!-- IF postrow.POSTER_AVATAR --><p class="avatar">{postrow.POSTER_AVATAR}</p><!-- ENDIF -->
    <!-- IF postrow.POSTER_STATUS --><p class="joined"><em>{L_STATUS}:</em> {postrow.POSTER_STATUS}</p><!-- ENDIF -->
    ...
    <!-- IF REPUTATION_ON --><p><em>{L_REPUTATION}:</em>&nbsp;<span id="reputation-{postrow.POSTER_ID}" style="color:{postrow.REPUTATION_COLOR}">{postrow.REPUTATION}</span>&nbsp;<a href="#" onclick="ajax.reputation({postrow.POSTER_ID}, 'add'); return false;" class="small">[+]</a>&nbsp;<a href="#" onclick="ajax.reputation({postrow.POSTER_ID}, 'del'); return false;" class="small">[-]</a></p><!-- ENDIF -->
	...
	<td class="message td2" rowspan="2">
		...
		<div class="post_head">
			<p style="float: left;<!-- IF TEXT_BUTTONS --> padding: 4px 0 3px;<!-- ELSE --> padding-top: 5px;<!-- ENDIF -->">
				...
				<a data-clipboard-text="{FULL_URL}{U_POST_URL}{postrow.POST_ID}#{postrow.POST_ID}" class="small copyElement" href="{U_POST_URL}{postrow.POST_ID}#{postrow.POST_ID}" title="{L_POST_LINK}">{postrow.POST_DATE} | #{postrow.POST_NUMBER}</a>
...
<tr>
	...
	<!-- ELSE -->
      ...
      <!-- IF $bb_cfg['scroll_to_top']['enabled'] -->
      <div class="mrg_4"><span onclick="$('html, body').animate({scrollTop: 0}, {$bb_cfg['scroll_to_top']['speed']})" style="cursor: pointer;" class="txtb">{L_BACK_TO_TOP}</span></div>
      <!-- ENDIF -->
		</div>
	<!-- ENDIF -->
