...
<table class="forumline">
	...
	<tr>
		<td class="row1"><span class="gen"><b>{L_USERFIELD}:</b>&nbsp;<select name="userfield_type"><option value="icq" selected="selected">{L_ICQ}</option><option value="skype">{L_SKYPE}</option><option value="twitter">{L_TWITTER}</option><option value="telegram">{L_TELEGRAM}</option><option value="vk">{L_VK}</option><option value="website">{L_WEBSITE}</option><option value="location">{L_LOCATION}</option><option value="interests">{L_INTERESTS}</option><option value="prov">{L_PROV}</option></select>&nbsp;<input class="post" type="text" name="userfield_value" maxlength="25" size="25" />&nbsp;<input type="submit" class="post2" name="search_userfield" value="{L_SEARCH}" /></span><br /><span class="small">{L_SEARCH_USERS_USERFIELD_EXPLAIN}</span></td>
	</tr>
	<tr>
		<td class="row2">&nbsp;</td>
	</tr>
  ...
  <tr>
    <td class="catBottom" style="padding: 0; height: 26px">
      <span class="med"><input type="reset" value="{L_RESET}" class="liteoption" /></span>
    </td>
  </tr>
</table>
</form>
