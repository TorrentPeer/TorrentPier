...
if (!isset($_REQUEST['dosearch'])) {
  ...
  switch ($mode) {
    ...
    case 'search_ip':
      ...
      if (\TorrentPier\Helpers\IPHelper::verify_ip($ip_address)) {
        $ip = \TorrentPier\Helpers\IPHelper::encode_ip($ip_address);
        $users[] = $ip;
      } else {
        bb_die($lang['SEARCH_INVALID_IP']);
      }
    ...
    case 'search_userfield':
      ...
      switch ($userfield_type) {
        ...
        case 'occupation':
          ...
          break;
        case 'prov':
          $text = sprintf($lang['SEARCH_FOR_USERFIELD_PROVIDER'], $text);
          $field = 'user_prov';
          break;
        default:
          bb_die($lang['SEARCH_INVALID']);
  ...
  for ($i = 0, $iMax = count($rowset); $i < $iMax; $i++) {
    ...
    $template->assign_block_vars('userrow', [
      ...
      'U_VIEWPOSTS' => BB_ROOT . "search.php?search_author=1&amp;uid={$rowset[$i]['user_id']}",
      'U_MANAGE' => BB_ROOT . 'profile.php?mode=editprofile&' . POST_USERS_URL . '=' . $rowset[$i]['user_id'] . '&admin=1',
