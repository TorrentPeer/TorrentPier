...
$size = \TorrentPier\Legacy\AttachMod\Attach::get_var('size', '');
$mode = \TorrentPier\Legacy\AttachMod\Attach::get_var('mode', '');
$e_mode = \TorrentPier\Legacy\AttachMod\Attach::get_var('e_mode', '');
...
if ($submit && $mode == 'extensions') {
  // Change Extensions ?
  $extension_change_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('extension_change_list', [0]);
  $extension_explain_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('extension_explain_list', ['']);
  $group_select_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('group_select', [0]);
  ...
  if ($num_rows > 0) {
    for ($i = 0, $iMax = count($extension_row); $i < $iMax; $i++) {
      if ($extension_row[$i]['comment'] != $extensions['_' . $extension_row[$i]['ext_id']]['comment'] || (int)$extension_row[$i]['group_id'] != (int)$extensions['_' . $extension_row[$i]['ext_id']]['group_id']) {
        ...
        $sql = 'UPDATE ' . BB_EXTENSIONS . ' SET ' . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_build_array('UPDATE', $sql_ary) . '
  ...
  // Delete Extension?
  $extension_id_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('extension_id_list', [0]);
  ...
  // Add Extension ?
  $extension = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_extension', '');
  $extension_explain = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_extension_explain', '');
  $extension_group = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_group_select', 0);
  ...
  if ($extension != '' && $add) {
    ...
    if (!$error) {
      ...
      if (!$error) {
        ...
        $sql = 'INSERT INTO ' . BB_EXTENSIONS . ' ' . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_build_array('INSERT', $sql_ary);
...
if ($mode == 'extensions') {
  ...
  if ($submit) {
    $template->assign_vars([
      'S_ADD_GROUP_SELECT' => \TorrentPier\Legacy\AttachMod\Selects::group_select('add_group_select', $extension_group)
    ]);
  } else {
    $template->assign_vars([
      'S_ADD_GROUP_SELECT' => \TorrentPier\Legacy\AttachMod\Selects::group_select('add_group_select')
    ]);
  }
  ...
  if ($num_extension_row > 0) {
    $extension_row = \TorrentPier\Legacy\AttachMod\Admin::sort_multi_array($extension_row, 'group_name', 'ASC');

    for ($i = 0; $i < $num_extension_row; $i++) {
      if ($submit) {
        $template->assign_block_vars('extension_row', [
          'EXT_ID' => $extension_row[$i]['ext_id'],
          'EXTENSION' => $extension_row[$i]['extension'],
          'EXTENSION_EXPLAIN' => $extension_explain_list[$i],
          'S_GROUP_SELECT' => \TorrentPier\Legacy\AttachMod\Selects::group_select('group_select[]', $group_select_list[$i])
        ]);
      } else {
        $template->assign_block_vars('extension_row', [
          'EXT_ID' => $extension_row[$i]['ext_id'],
          ...
          'S_GROUP_SELECT' => \TorrentPier\Legacy\AttachMod\Selects::group_select('group_select[]', $extension_row[$i]['group_id'])
        ]);
      }
    }
  }
...
if ($submit && $mode == 'groups') {
  ...
  $group_change_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('group_change_list', [0]);
  $extension_group_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('extension_group_list', ['']);
  $group_allowed_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('allowed_list', [0]);
  $download_mode_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('download_mode_list', [0]);
  $category_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('category_list', [0]);
  $upload_icon_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('upload_icon_list', ['']);
  $filesize_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('max_filesize_list', [0]);
  $size_select_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('size_select_list', ['']);
  ...
  for ($i = 0, $iMax = count($group_change_list); $i < $iMax; $i++) {
    ...
    $sql = 'UPDATE ' . BB_EXTENSION_GROUPS . ' SET ' . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_build_array('UPDATE', $sql_ary) . '
  ...
  $group_id_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('group_id_list', [0]);
  ...
  // Add Extensions?
  $extension_group = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_extension_group', '');
  $download_mode = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_download_mode', 0);
  $cat_id = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_category', 0);
  $upload_icon = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_upload_icon', '');
  $filesize = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_max_filesize', 0);
  $size_select = \TorrentPier\Legacy\AttachMod\Attach::get_var('add_size_select', '');
  ...
  if ($extension_group != '' && $add) {
    ...
    if (!$error) {
      ...
      $sql = 'INSERT INTO ' . BB_EXTENSION_GROUPS . ' ' . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_build_array('INSERT', $sql_ary);
...
if ($mode == 'groups') {
  ...
  $viewgroup = \TorrentPier\Legacy\AttachMod\Attach::get_var(POST_GROUPS_URL, 0);

  $template->assign_vars([
    'TPL_ATTACH_EXTENSION_GROUPS' => true,
    'ADD_GROUP_NAME' => $extension_group ?? '',
    'MAX_FILESIZE' => $max_add_filesize,
    'S_FILESIZE' => \TorrentPier\Legacy\AttachMod\Selects::size_select('add_size_select', $size),
    'S_ADD_DOWNLOAD_MODE' => \TorrentPier\Legacy\AttachMod\Selects::download_select('add_download_mode'),
    'S_SELECT_CAT' => \TorrentPier\Legacy\AttachMod\Selects::category_select('add_category'),
  ...
  for ($i = 0; $i < $num_extension_group; $i++) {
    ...
    $template->assign_block_vars('grouprow', [
      ...
      'S_SELECT_CAT' => \TorrentPier\Legacy\AttachMod\Selects::category_select('category_list[]', $extension_group[$i]['group_id']),
      'S_DOWNLOAD_MODE' => \TorrentPier\Legacy\AttachMod\Selects::download_select('download_mode_list[]', $extension_group[$i]['group_id']),
      'S_FILESIZE' => \TorrentPier\Legacy\AttachMod\Selects::size_select('size_select_list[]', $size_format),
    ...
    if ($viewgroup && $viewgroup == $extension_group[$i]['group_id']) {
      ...
      for ($j = 0; $j < $num_extension; $j++) {
        $template->assign_block_vars('grouprow.extensionrow', [
...
if ($e_mode == 'perm') {
  $group = \TorrentPier\Legacy\AttachMod\Attach::get_var('e_group', 0);
...
if ($add_forum && $e_mode == 'perm' && $group) {
  $add_forums_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('entries', [0]);
  ...
  if (!$add_all_forums) {
    ...
    if (trim($row['forum_permissions']) == '') {
      ...
      $auth_p = \TorrentPier\Legacy\AttachMod\Attach::auth_unpack($row['forum_permissions']);
    }
    ...
    $auth_bitstream = \TorrentPier\Legacy\AttachMod\Attach::auth_pack($auth_p);

    $sql = 'UPDATE ' . BB_EXTENSION_GROUPS . " SET forum_permissions = '" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($auth_bitstream) . "' WHERE group_id = " . (int)$group;
...
if ($delete_forum && $e_mode == 'perm' && $group) {
  $delete_forums_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('entries', [0]);
  ...
  $auth_p2 = \TorrentPier\Legacy\AttachMod\Attach::auth_unpack(trim($row['forum_permissions']));
  $auth_p = [];
  ...
  $auth_bitstream = (count($auth_p) > 0) ? \TorrentPier\Legacy\AttachMod\Attach::auth_pack($auth_p) : '';
...
if ($e_mode == 'perm' && $group) {
  ...
  if ($allowed_forums == '') {
    ...
    $forum_p = [];
    $act_id = 0;
    $forum_p = \TorrentPier\Legacy\AttachMod\Attach::auth_unpack($allowed_forums);
  ...
  while ($row = DB()->sql_fetchrow($f_result)) {
    ...
    for ($i = 0; $i < $num_rows; $i++) {
      $allowed_forums = \TorrentPier\Legacy\AttachMod\Attach::auth_unpack(trim($rows[$i]['forum_permissions']));
  ...
  if (is_countable($empty_perm_forums) && (count($empty_perm_forums) > 0)) {
