...
if (!empty($setmodules)) {
  $module['USERS']['BAN_CONTROL'] = basename(__FILE__);
  return;
}
...
if (isset($_POST['submit'])) {
  ...
  if (isset($_POST['ban_ip'])) {
    ...
    foreach ($ip_list_temp as $ip) {
      if (\TorrentPier\Helpers\IPHelper::verify_ip($ip)) {
        $ip_list[] = \TorrentPier\Helpers\IPHelper::encode_ip($ip);
      }
    }
  }
  ...
  for ($i = 0, $iMax = count($banlist); $i < $iMax; $i++) {
    ...
    if (!empty($banlist[$i]['ban_ip'])) {
      $ban_ip = str_replace('255', '*', \TorrentPier\Helpers\IPHelper::decode_ip($banlist[$i]['ban_ip']));
