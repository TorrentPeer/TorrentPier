...
while ($row = DB()->sql_fetchrow($result)) {
  ...
  $new_attach[$config_name] = \TorrentPier\Legacy\AttachMod\Attach::get_var($config_name, trim($attach_config[$config_name]));
  ...
  if ($submit && ($mode == 'manage' || $mode == 'cats')) {
    ...
    if ($config_name == 'max_filesize') {
      ...
      $sql = 'UPDATE ' . BB_ATTACH_CONFIG . "
				SET	config_value = '" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($new_attach[$config_name]) . "'
				WHERE config_name = '" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($config_name) . "'";
    } else {
      $sql = 'UPDATE ' . BB_ATTACH_CONFIG . "
				SET	config_value = '" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($new_attach[$config_name]) . "'
				WHERE config_name = '" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($config_name) . "'";
...
$select_size_mode = \TorrentPier\Legacy\AttachMod\Selects::size_select('size', $size);
$select_quota_size_mode = \TorrentPier\Legacy\AttachMod\Selects::size_select('quota_size', $quota_size);
$select_pm_size_mode = \TorrentPier\Legacy\AttachMod\Selects::size_select('pm_size', $pm_size);

// Search Imagick
if ($search_imagick) {
  $imagick = \TorrentPier\Legacy\AttachMod\Thumbs::$imagick;
  ...
  if ($imagick != 'none') {
    if (!false !== stripos(PHP_OS, "WIN")) {
      ...
      if (!@file_exists(\TorrentPier\Legacy\AttachMod\Attach::amod_realpath($path))) {
        $imagick = $path;
      }
    }
  }

  if (!@file_exists(\TorrentPier\Legacy\AttachMod\Attach::amod_realpath(trim($imagick)))) {
...
if ($check_upload) {
  ...
  if (!@file_exists(\TorrentPier\Legacy\AttachMod\Attach::amod_realpath($upload_dir))) {
    $error = true;
  ...
  if (!$error) {
    if (!($fp = @fopen($upload_dir . '/0_000000.000', 'wb'))) {
      ...
      @fclose($fp);
      \TorrentPier\Legacy\AttachMod\Attach::unlink_attach($upload_dir . '/0_000000.000');
    }
  }
...
if ($mode == 'manage') {
  $template->assign_vars([
    ...
    'S_DEFAULT_UPLOAD_LIMIT' => \TorrentPier\Legacy\AttachMod\Selects::default_quota_limit_select('default_upload_quota', (int)trim($new_attach['default_upload_quota'])),
    'S_DEFAULT_PM_LIMIT' => \TorrentPier\Legacy\AttachMod\Selects::default_quota_limit_select('default_pm_quota', (int)trim($new_attach['default_pm_quota'])),
...
if ($mode == 'cats') {
  ...
  if (!\TorrentPier\Legacy\AttachMod\Thumbs::is_imagick() && !@extension_loaded('gd')) {
...
if ($submit && $mode == 'quota') {
  // Change Quota Limit
  $quota_change_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('quota_change_list', [0]);
  $quota_desc_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('quota_desc_list', ['']);
  ...
  for ($i = 0, $iMax = count($quota_change_list); $i < $iMax; $i++) {
    ...
    $sql = 'UPDATE ' . BB_QUOTA_LIMITS . "
			SET quota_desc = '" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($quota_desc_list[$i]) . "', quota_limit = " . (int)$filesize_list[$i] . '
  ...
  // Delete Quota Limits
  $quota_id_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('quota_id_list', [0]);
  ...
  // Add Quota Limit ?
  $quota_desc = \TorrentPier\Legacy\AttachMod\Attach::get_var('quota_description', '');
  ...
  if ($quota_desc != '' && $add) {
    ...
    if (!$error) {
      ...
      $sql = 'INSERT INTO ' . BB_QUOTA_LIMITS . " (quota_desc, quota_limit)
			VALUES ('" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($quota_desc) . "', " . (int)$filesize . ')';
...
if ($mode == 'quota' && $e_mode == 'view_quota') {
  $quota_id = \TorrentPier\Legacy\AttachMod\Attach::get_var('quota_id', 0);
