...
if (isset($_GET['pane']) && $_GET['pane'] == 'left') {
  ...
  $template->assign_vars([
    ...
    'U_FORUM_INDEX' => BB_ROOT . 'index.php',
  ...
  if (isset($_GET['users_online'])) {
    ...
    if (is_countable($onlinerow_reg)) {
      ...
      for ($i = 0, $iMax = count($onlinerow_reg); $i < $iMax; $i++) {
        if (!in_array($onlinerow_reg[$i]['user_id'], $reg_userid_ary)) {
          ...
          $reg_ip = \TorrentPier\Helpers\IPHelper::decode_ip($onlinerow_reg[$i]['session_ip']);
          ...
          $template->assign_block_vars('reg_user_row', [
            ...
            'IP_ADDRESS' => $reg_ip,
            'USER_AGENT' => $onlinerow_reg[$i]['session_agent'],
    ...
    // Guest users
    if (is_countable($onlinerow_guest)) {
      $guest_users = 0;
      ...
      for ($i = 0, $iMax = count($onlinerow_guest); $i < $iMax; $i++) {
        ...
        $guest_ip = \TorrentPier\Helpers\IPHelper::decode_ip($onlinerow_guest[$i]['session_ip']);
