...
// Init Variables
$start = \TorrentPier\Legacy\AttachMod\Attach::get_var('start', 0);
$sort_order = \TorrentPier\Legacy\AttachMod\Attach::get_var('order', 'ASC');
$sort_order = ($sort_order == 'ASC') ? 'ASC' : 'DESC';
$mode = \TorrentPier\Legacy\AttachMod\Attach::get_var('mode', '');
$view = \TorrentPier\Legacy\AttachMod\Attach::get_var('view', '');
$uid = isset($_POST['u_id']) ? \TorrentPier\Legacy\AttachMod\Attach::get_var('u_id', 0) : \TorrentPier\Legacy\AttachMod\Attach::get_var('uid', 0);
...
if (is_countable($mode_types_text) && (count($mode_types_text) > 0)) {
...
$delete_id_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('delete_id_list', [0]);
...
if ($confirm && count($delete_id_list) > 0) {
  ...
  \TorrentPier\Legacy\AttachMod\Delete::delete_attachment(0, $delete_id_list);
...
if ($submit_change && $view === 'attachments') {
  $attach_change_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('attach_id_list', [0]);
  $attach_comment_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('attach_comment_list', ['']);
  $attach_download_count_list = \TorrentPier\Legacy\AttachMod\Attach::get_var('attach_count_list', [0]);
    ...
    if (isset($attachments['_' . $attachrow['attach_id']])) {
      if ($attachrow['comment'] != $attachments['_' . $attachrow['attach_id']]['comment'] || $attachrow['download_count'] != $attachments['_' . $attachrow['attach_id']]['download_count']) {
        $sql = 'UPDATE ' . BB_ATTACHMENTS_DESC . "
					SET comment = '" . \TorrentPier\Legacy\AttachMod\Attach::attach_mod_sql_escape($attachments['_' . $attachrow['attach_id']]['comment']) . "', download_count = " . (int)$attachments['_' . $attachrow['attach_id']]['download_count'] . '
...
if ($view === 'attachments') {
  ...
  if ($user_based) {
    ...
    $attachments = \TorrentPier\Legacy\AttachMod\Admin::search_attachments($order_by, $total_rows);
  }
