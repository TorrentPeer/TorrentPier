...
if (!empty($setmodules)) {
  if (IS_SUPER_ADMIN) {
    $module[$bb_cfg['tp_name']]['CRON'] = basename(__FILE__) . '?mode=list';
  }
  return;
}
...
$jobs = isset($_POST['select']) ? (array)$_POST['select'] : '';
  ...
  case 'list':
    ...
    foreach ($sql as $i => $row) {
      $template->assign_block_vars('list', [
        ...
        'JOB_ID' => $i + 1,
        'CRON_ID' => $row['cron_id'],
        'CRON_ACTIVE' => $row['cron_active'] ? '<img src="' . $images['icon_run'] . '" alt="' . $lang['YES'] . '" />' : '<img src="' . $images['icon_delete'] . '" alt="' . $lang['NO'] . '" />',
...
if ($submit) {
  if ($_POST['mode'] == 'list') {
    if ($cron_action == 'run' && is_countable($jobs)) {
      \TorrentPier\Legacy\Admin\Cron::run_jobs($jobs);
    } elseif ($cron_action == 'delete' && is_countable($jobs)) {
      \TorrentPier\Legacy\Admin\Cron::delete_jobs($jobs);
    } elseif (($cron_action == 'disable' || $cron_action == 'enable') && is_countable($jobs)) {
