  ...
  foreach ($tor_search_tracker as $statuses_part) {
    ...
    foreach ($statuses_part as $status_id => $status_on) {
      if ($status_on) {
        ...
        $statuses .= '<td><p class="chbox"><label><input type="checkbox" name="status[]" value="' . $status_id . '"' . $checked . '>' . $bb_cfg['tor_icons'][$status_id] . ' ' . $lang['TOR_STATUS_NAME'][$status_id] . '</label></p></td>';
      }
    }
    $statuses .= '</tr>';
  }
